import 'package:flutter/material.dart';
import 'package:flutter_svg/svg.dart';
import 'package:camerawesome/camerawesome_plugin.dart';
import 'package:camerawesome/pigeon.dart';
import 'package:image_picker/image_picker.dart';
import 'package:loading_animation_widget/loading_animation_widget.dart';
import 'dart:ui';
import 'dart:async';
import 'package:provider/provider.dart';
import '../user_provider.dart';
import '../components/safe_bar.dart';
import '../components/nav_bar.dart';
import './send_poke.dart';

class Cam extends StatefulWidget {
  const Cam({super.key});

  @override
  State<Cam> createState() => _Cam();
}

class _Cam extends State<Cam> with TickerProviderStateMixin {
  dynamic saveConfig;
  dynamic cameraState;
  dynamic cameraReady = false;
  dynamic refreshCamera = false;
  dynamic awesomePath; // for video pokes its weird ik

  dynamic touch;
  dynamic touchBounce;

  dynamic poking = false;

  dynamic ani;
  dynamic blinkingColor;
  dynamic _radius = 55.0;

  @override
  initState() {
    super.initState();

    ani = AnimationController(duration: Duration(seconds: 1), vsync: this)
      ..repeat(reverse: true);

    blinkingColor = ColorTween(
      begin: Color(0xff00c8ff).withAlpha(0),
      end: Color(0xff00c8ff).withAlpha(1),
    ).animate(ani);
  }

  dynamic isRecording = false;
  dynamic max = 5; // only allow 5secs snap (vercel back limit) ;-; (fix maybe)
  dynamic timeRn = 0;

  poke(file, whatIs) async {
    await Navigator.push(
      context,
      MaterialPageRoute(
        builder: (context) => SendPoke(file: file, whatis: whatIs),
      ),
    );

    setState(() {
      poking = false;
      _radius = 55.0;
      if (whatIs != 'img') {
        timeRn = 0;
      }
    });
  }

  @override
  Widget build(BuildContext context) {
    dynamic usr = Provider.of<UserPro>(context);
    dynamic setOptions = usr.setOptions;
    dynamic options = usr.options;

    stopRecording() async {
      if (poking) return;
      if (!isRecording) return;
      setState(() {
        poking = true;
        isRecording = false;
      });

      await cameraState.stopRecording();
      poke(XFile(awesomePath), 'mp4');
    }

    updateprogress() {
      if (!isRecording) return;
      if (poking) return;

      if (timeRn > max) {
        stopRecording();
        return;
      }

      Future.delayed(Duration(seconds: 1), () {
        if (!isRecording) return;
        setState(() {
          timeRn++;
        });
        updateprogress();
      });
    }

    startRecording() async {
      if (!cameraReady) return;
      if (poking) return;
      if (isRecording) return;

      cameraState.setState(CaptureMode.video);

      WidgetsBinding.instance.addPostFrameCallback((_) async {
        dynamic capture = await cameraState.startRecording();
        setState(() {
          awesomePath = capture.path;
        });
      }); // otherwise gives error ;-;

      setState(() {
        _radius = 20.0;
        isRecording = true;
        timeRn = 0;
      });
      updateprogress();
    }

    toggleFlash() async {
      if (isRecording || !cameraReady || poking) return; // otherwise breaks ;-;
      cameraState.sensorConfig.setFlashMode(
        options['isFlash'] ? FlashMode.none : FlashMode.always,
      );
      await setOptions({'isFlash': !options['isFlash']});
    }

    toggleNoise() async {
      if (isRecording || !cameraReady || poking) return; // same ;-;
      setState(() {
        cameraReady = false;
        refreshCamera = true;
      });

      WidgetsBinding.instance.addPostFrameCallback((_) {
        setState(() {
          refreshCamera = false;
        });
      });

      await setOptions({'isNoise': !options['isNoise']});
    }

    toggleCamera() async {
      if (isRecording || !cameraReady || poking) return; // sme

      setState(() {
        cameraReady = false;
      });

      cameraState.switchCameraSensor();

      setState(() {
        cameraReady = true;
      });
    }

    takePhoto() async {
      if (!cameraReady) return;
      if (isRecording) return; // otherwise brokey
      if (poking) return;
      setState(() {
        poking = true;
      });

      cameraState.setState(CaptureMode.photo);

      WidgetsBinding.instance.addPostFrameCallback((_) async {
        dynamic capture = await cameraState.takePhoto();
        poke(XFile(capture.path), 'img');
      }); // otherwise gives error ;-;
    }

    return Scaffold(
      backgroundColor: Colors.black,
      body: SafeArea(
        top: false,
        child: Stack(
          fit: StackFit.expand,
          children: [
            refreshCamera
                ? SizedBox.shrink()
                : CameraAwesomeBuilder.custom(
                  sensorConfig: SensorConfig.single(
                    flashMode:
                        options['isFlash'] ? FlashMode.always : FlashMode.none,
                  ),
                  saveConfig: SaveConfig.photoAndVideo(
                    initialCaptureMode: CaptureMode.photo,
                    mirrorFrontCamera: true,
                    videoOptions: VideoOptions(enableAudio: options['isNoise']),
                  ),
                  builder: (state, preview) {
                    cameraState = state;

                    if (!cameraReady) {
                      WidgetsBinding.instance.addPostFrameCallback((_) {
                        setState(() => cameraReady = true);
                      }); // otherwise gives error ;-;
                    }

                    return GestureDetector(
                      behavior: HitTestBehavior.opaque,
                      onDoubleTap: toggleCamera,
                      child: const SizedBox.expand(),
                    );
                  },
                ),
            cameraReady
                ? SizedBox.shrink()
                : SizedBox.expand(
                  child: ColoredBox(color: Colors.black),
                ), // hides any glitches :3
            Positioned(
              bottom: 96, // navBar + 30
              left: 0,
              right: 0,
              child: Center(
                child:
                    poking
                        ? LoadingAnimationWidget.beat(
                          color: Color(0xff00c8ff),
                          size: 100,
                        )
                        : GestureDetector(
                          behavior: HitTestBehavior.opaque, // needed to hit
                          child: Listener(
                            // gesture in gesture won't work
                            onPointerDown: (_) {
                              touch = DateTime.now();
                              touchBounce = Timer(
                                Duration(milliseconds: 500),
                                () {
                                  startRecording(); // record if 500ms <
                                },
                              );
                            },
                            onPointerUp: (_) {
                              touchBounce?.cancel();

                              dynamic duration = DateTime.now().difference(
                                touch,
                              );

                              if (duration < Duration(milliseconds: 500)) {
                                takePhoto(); // touch
                              } else {
                                // long touch
                                stopRecording();
                              }
                            },
                            child: AnimatedContainer(
                              curve: Curves.easeInOut,
                              duration: Duration(seconds: 1),
                              height: 110,
                              width: 110,
                              decoration: BoxDecoration(
                                color:
                                    _radius == 55.0
                                        ? Colors.white
                                        : Color(0xff00c8ff),
                                borderRadius: BorderRadius.circular(_radius),
                              ),
                            ),
                          ),
                        ),
              ),
            ),
            Positioned(
              top: MediaQuery.of(context).padding.top + 180, // safeBar + 120
              right: 10,
              child: ClipRRect(
                borderRadius: BorderRadius.circular(30),
                child: BackdropFilter(
                  filter: ImageFilter.blur(sigmaX: 10, sigmaY: 10),
                  child: Column(
                    children: [
                      ElevatedButton(
                        onPressed: toggleFlash,
                        style: ElevatedButton.styleFrom(
                          backgroundColor: Colors.transparent,
                          shadowColor: Colors.transparent,
                          elevation: 0,
                          padding: EdgeInsets.all(5),
                        ),
                        child: SvgPicture.asset(
                          options['isFlash']
                              ? 'images/flash-on.svg'
                              : 'images/flash-off.svg',
                          height: 30,
                          width: 30,
                          colorFilter: ColorFilter.mode(
                            Colors.white,
                            BlendMode.srcIn,
                          ),
                          alignment: Alignment.center,
                        ),
                      ),
                      SizedBox(height: 2),
                      ElevatedButton(
                        onPressed: toggleNoise,
                        style: ElevatedButton.styleFrom(
                          backgroundColor: Colors.transparent,
                          shadowColor: Colors.transparent,
                          elevation: 0,
                          padding: EdgeInsets.all(5),
                        ),
                        child: SvgPicture.asset(
                          options['isNoise']
                              ? 'images/blur-on.svg'
                              : 'images/blur-off.svg',
                          height: 30,
                          width: 30,
                          colorFilter: ColorFilter.mode(
                            Colors.white,
                            BlendMode.srcIn,
                          ),
                          alignment: Alignment.center,
                        ),
                      ),
                    ],
                  ),
                ),
              ),
            ),
            Positioned(
              top: MediaQuery.of(context).padding.top + 90, // safeBar + 30
              left: 0,
              right: 0,
              child:
                  timeRn != 0
                      ? Center(
                        child: Row(
                          crossAxisAlignment: CrossAxisAlignment.center,
                          mainAxisAlignment: MainAxisAlignment.center,
                          children: [
                            AnimatedBuilder(
                              animation: blinkingColor,
                              builder: (context, child) {
                                return Container(
                                  width: 10,
                                  height: 10,
                                  decoration: BoxDecoration(
                                    color: blinkingColor.value,
                                    shape: BoxShape.circle,
                                  ),
                                );
                              },
                            ),
                            SizedBox(width: 10),
                            Text(
                              '00:${timeRn > 9 ? timeRn : '0$timeRn'}',
                              style: TextStyle(
                                color: Colors.white,
                                fontWeight: FontWeight.w900,
                                fontSize: 16,
                              ),
                            ),
                          ],
                        ),
                      )
                      : SizedBox.shrink(),
            ),
            SafeBar(title: 'Poke'),
            NavBar(dontOpen: 'cam'),
          ],
        ),
      ),
    );
  }

  @override
  dispose() {
    ani.dispose();
    super.dispose();
  }
}
