...
class Gallery extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    // apply datify & group by date
    List<Map<dynamic, dynamic>> grouped = _groupByDate(allPokes);
  ...
  // group pokes by datified date
  List<Map<dynamic, dynamic>> _groupByDate(List<dynamic> pokes) {
    List<Map<dynamic, dynamic>> grouped = [];
    dynamic seenDates = <dynamic>{};

    for (dynamic poke in pokes) {
      dynamic createdAt = poke['createdAt'];
      dynamic date = _datify(createdAt);
      if (!seenDates.contains(date)) {
        dynamic dayPokes =
            pokes.where((p) => _datify(p['createdAt']) == date).toList();
  ...
  dynamic _datify(dynamic isoDate) {
    dynamic date = DateTime.parse(isoDate).toLocal();
    dynamic now = DateTime.now();
    dynamic today = DateTime(now.year, now.month, now.day);
    dynamic yesterday = today.subtract(Duration(days: 1));
    dynamic checkDate = DateTime(date.year, date.month, date.day);

    if (checkDate == today) return 'Tday';
    if (checkDate == yesterday) return 'Yday';

    // 'Jul 05' or 'Jul 05/25' dep on year
    dynamic newYear = date.year != now.year;
    dynamic fmt = newYear ? DateFormat('MMM dd/yy') : DateFormat('MMM dd');
